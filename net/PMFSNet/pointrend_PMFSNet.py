...
class PMFSNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=4, dim="2d", scaling_version="TINY",
        ...
        if scaling_version == "BASIC":
            ...
            self.upsample_1 = torch.nn.Upsample(scale_factor=2, mode=upsample_mode, align_corners=False)
            self.upsample_2 = torch.nn.Upsample(scale_factor=4, mode=upsample_mode,  align_corners=False)
        ...
        self.upsample_out = torch.nn.Upsample(scale_factor=2, mode=upsample_mode,  align_corners=False)
    ...
    def forward(self, x):
        if self.scaling_version == "BASIC":
            ...
            x2, skip2 = self.down_convs[1](x1)  #x2  64-64-64
            x3, skip3 = self.down_convs[2](x2)

            x3 = self.Global([x1, x2, x3]) #tiny时候101-32*32  small184-32*32
            skip3 = self.bottle_conv(torch.cat([x3, skip3], dim=1))#tiny时候24-32*32  small24-32*32

            skip2 = self.upsample_1(skip2)#tiny时候24-128*128   small 24-128*128
            skip3 = self.upsample_2(skip3)#tiny时候24-128*128    small 24-128*128

            out = self.out_conv(torch.cat([skip1, skip2, skip3], dim=1))
            out = self.upsample_out(out)

        return {'up2': skip3, 'coarse':out}  #原始{'up2': skip3, 'coarse':out}   之后挑选 {'up2':x2, 'coarse':out}
...
if __name__ == '__main__':
    ...
    model = PMFSNet(in_channels=3, out_channels=2, dim="2d", scaling_version="TINY")
    out=model(x)
