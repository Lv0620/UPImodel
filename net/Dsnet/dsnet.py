...
def build_dsnet_model(model_name, num_classes):
    """
    根据模型名字和类别数构建 DSNet 模型。

    Args:
        model_name (str): 模型名称，例如 'dsnet_head128', 'dsnet_head64', 'dsnet_head256', 'dsnet_m'
        num_classes (int): 输出类别数

    Returns:
        model (DSNet): 构建好的 DSNet 模型
    """
    if model_name == 'dsnet_head128':
        model = DSNet(m=2, n=2, num_classes=num_classes, planes=32, name='s128', augment=False)
    elif model_name == 'dsnet_head64':
        model = DSNet(m=2, n=2, num_classes=num_classes, planes=32, name='s64', augment=False)
    elif model_name == 'dsnet_head256':
        model = DSNet(m=2, n=2, num_classes=num_classes, planes=32, name='s256', augment=False)
    elif model_name == 'dsnet_m':
        model = DSNet(m=2, n=3, num_classes=num_classes, planes=64, name='m', augment=False)
    else:
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available():
        x = x.to('cuda')
    model =build_dsnet_model("dsnet_head128", 4)
    model = model.to('cuda')
    with torch.no_grad():
        pred = model(x)
    print(pred.size())
