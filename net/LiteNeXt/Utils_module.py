...
class CTblock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3) -> None:
        ...
        self.mixspatial = nn.Sequential(
            nn.Conv2d(in_channel, in_channel, kernel_size, padding=int((kernel_size-1)//2), groups=in_channel),
...
class UpDownstream(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.ac(x)
        bn, c, h, w = x.shape
        x = F.interpolate(x, size=(int(h*self.scale), int(w*self.scale)), mode="bilinear", align_corners=False)
        return x
...
class SimpleAttentionDC(nn.Module):
    def __init__(self, scale, in_channel, out_channel) -> None:
        ...
        self.axial_conv = nn.Sequential(
            nn.Conv2d(out_channel*4, out_channel*4, (7,1), dilation=(3,1), padding=(9,0)),
            nn.Conv2d(out_channel*4, out_channel*4, (1,7), dilation=(1,3), padding=(0,9)),
...
class MixSpatial(nn.Module):
    def __init__(self, in_channel, p=0.1):
        ...
        self.conv7x7 = nn.Sequential(
            nn.Conv2d(in_channel, in_channel, 7, dilation=5, padding=15, groups=in_channel),
            nn.Dropout2d(p),
            nn.BatchNorm2d(in_channel)
        )
        self.conv5x5 = nn.Sequential(
            nn.Conv2d(in_channel, in_channel, 5, dilation=3, padding=6, groups=in_channel),
            nn.Dropout2d(p),
            nn.BatchNorm2d(in_channel)
        )
        self.conv3x3 = nn.Sequential(
            nn.Conv2d(in_channel, in_channel, 3, dilation=1, padding=1, groups=in_channel),
            nn.Dropout2d(p),
            nn.BatchNorm2d(in_channel)
        )
        self.gelu = nn.GELU()
...
class SMmodule(nn.Module):
    def __init__(self, in_channel, expand = 2, kernel_size=7, kernel_local=3, drop_rate=0.1, dilate =1):
        ...
        self.conv = nn.Conv2d(in_channel, in_channel, kernel_size, padding=int((kernel_size-1)*dilate//2), groups=in_channel, dilation=dilate)
        self.conv1x1 = nn.Conv2d(in_channel, in_channel*expand, 1)
        self.conv_local = nn.Conv2d(in_channel*expand, in_channel, kernel_local, padding=int((kernel_local-1)//2))
...
class SMmoduleplus(nn.Module):
    def __init__(self, in_channel, expand = 2, kernel_size=7, kernel_local=3, drop_rate=0.1, dilate =1):
        ...
        self.conv_local = nn.Conv2d(in_channel, in_channel, kernel_local, padding=int((kernel_local-1)//2))
        self.drp = DropBlock2D(block_size=3, drop_prob=drop_rate)
        self.ln1 = Layernorm(in_channel)
...
class SMmoduleplusplus(nn.Module):
    def __init__(self, in_channel, kernel_size=7, kernel_local=3, drop_rate=0.1, dilate =1):
        ...
        self.conv = nn.Conv2d(in_channel, in_channel, kernel_size, padding=int(dilate*(kernel_size-1)//2), groups=in_channel, dilation=dilate)
        self.conv1x1 = nn.Conv2d(in_channel, in_channel, 1)
        self.conv_local = nn.Conv2d(in_channel, in_channel, kernel_local, groups=1, padding=int((kernel_size-1)//2))
        self.drp = DropBlock2D(block_size=3, drop_prob=drop_rate)
        self.ln1 = Layernorm(in_channel)
        self.ln2 = Layernorm(in_channel)
        self.ln3 = Layernorm(in_channel)
...
class SMmodulesimple(nn.Module):
    def __init__(self, in_channel, kernel_size=3, drop_rate=0.01, dilate =1):
        ...
        self.conv_local = nn.Conv2d(in_channel, in_channel, kernel_size, groups=1, dilation=dilate, padding=int((kernel_size-1)*dilate//2))
        self.conv1x1 = nn.Conv2d(in_channel, in_channel, 1)
        self.drp = DropBlock2D(block_size=3, drop_prob=drop_rate)
        self.ln1 = Layernorm(in_channel)
        self.ln2 = Layernorm(in_channel)
